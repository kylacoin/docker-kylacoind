...
KCN_IMAGE=${KCN_IMAGE:-kylacoin/kylacoind}
...
docker kill kylacoind-node 2>/dev/null || true
docker rm kylacoind-node 2>/dev/null || true
stop docker-kylacoind 2>/dev/null || true

# Always pull remote images to avoid caching issues
if [ -z "${KCN_IMAGE##*/*}" ]; then
    docker pull $KCN_IMAGE
fi

# Initialize the data container
docker volume create --name=kylacoind-data
docker run -v kylacoind-data:/kylacoin --rm $KCN_IMAGE kcn_init

# Start kylacoind via upstart and docker
curl https://raw.githubusercontent.com/kylacoin/docker-kylacoind/master/upstart.init > /etc/init/docker-kylacoind.conf
start docker-kylacoind

set +ex
echo "Resulting kylacoin.conf:"
docker run -v kylacoind-data:/kylacoin --rm $KCN_IMAGE cat /kylacoin/.kylacoin/kylacoin.conf
